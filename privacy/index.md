---
layout: page
title: 公認会計士 短答トレーナー プライバシーポリシー
permalink: /privacy/
---
...
| IPアドレス、user agent | Firebase Authentication、Cloud Run、backend | 認証要求の処理とセキュリティ保護。下記の利用者ID由来のカウンターには、入力としても保存情報としても使わない |
...
バージョン0.1.0のビルド9以降では、誤植報告を送信前に端末へ保存し、サーバーから成功応答を受け取った後に削除します。通信・認証・入力エラーなどで未送信の報告は端末に残ります。入力エラーが確定した報告は、自動再送を止めて保持します。

ただし、再送停止のための情報を端末に保存できなかった場合は、報告を保持したまま、次の報告送信時に再送する可能性があります。送信成功後でも端末からの削除に失敗した場合は、次の報告送信時の再送・削除まで残る可能性があります。次回送信がない場合や自動再送を止めた場合は、アプリ削除またはアプリ内のアカウント削除の完了まで保持されます。サーバーで受信した報告は、上記のとおり受信日時から180日後の次の日次処理で削除します。
